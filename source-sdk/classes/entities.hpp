...
public:
	...
	void set_spotted(bool state) {
		if (!this) return;
		static uintptr_t offset = 0;
		offset = netvar_manager::get_net_var(fnv::hash("DT_BaseEntity"), fnv::hash("m_bSpotted"));
		write(offset, state);
	}
...
public:
	...
	void set_observer(int index){
		if (!this) return;
		static uintptr_t offset = 0;
		offset = netvar_manager::get_net_var(fnv::hash("DT_BasePlayer"), fnv::hash("m_hObserverTarget"));
		write(offset, index);
	}
	...
	bool can_see_player_pos(player_t * player, const vec3_t & pos) {
		...
		return tr.entity == player || tr.flFraction > 0.93f;
	}
	bool aiming_at(player_t* player) {
		trace_t tr;
		ray_t ray;
		trace_filter filter;
		filter.skip = this;
		vec3_t pos = player->origin();
		auto start = get_eye_pos();
		auto dir = (pos - start).normalized();
	...
	void set_flash(int duration) {
		if (!this) return;
		static uintptr_t offset = 0;
		offset = netvar_manager::get_net_var(fnv::hash("DT_CSPlayer"), fnv::hash("m_flFlashDuration"));
		write(offset, duration);
	}
	void set_view_punch(vec3_t punch) {
		if (!this) return;
		static uintptr_t offset = 0;
		offset = netvar_manager::get_net_var(fnv::hash("DT_BasePlayer"), fnv::hash("m_viewPunchAngle"));
		write(offset, punch);
	}
	void set_flash_alpha(float alpha) {
		if (!this) return;
		static uintptr_t offset = 0;
		offset = netvar_manager::get_net_var(fnv::hash("DT_CSPlayer"), fnv::hash("m_flFlashMaxAlpha"));
		write(offset, alpha);
	}
	void set_view_fov(int amount) {
		if (!this) return;
		static uintptr_t offset = 0;
		offset = netvar_manager::get_net_var(fnv::hash("DT_CSPlayer"), fnv::hash("m_iDefaultFOV"));
		write(offset, amount);
	}
	void set_view_fov_2(int amount) {
		if (!this) return;
		static uintptr_t offset = 0;
		offset = netvar_manager::get_net_var(fnv::hash("DT_BasePlayer"), fnv::hash("m_iFOV"));
		write(offset, amount);
	}
