...
unsigned long render::fonts::button_font;

void render::initialize() {
	render::fonts::watermark_font = interfaces::surface->font_create();
	render::fonts::button_font = interfaces::surface->font_create();
	interfaces::surface->set_font_glyph(render::fonts::watermark_font, "Tahoma", 12, 500, 0, 0, font_flags::fontflag_dropshadow);
	interfaces::surface->set_font_glyph(render::fonts::button_font, "Tahoma", 12, 500, 0, 0, font_flags::fontflag_none);
...
void render::draw_border_line_vertical (std::int32_t x1, std::int32_t y1, std::int32_t x2, std::int32_t y2, color colour) {
	interfaces::surface->set_drawing_color(colour.r, colour.g, colour.b, colour.a);
	interfaces::surface->draw_line(x1, y1, x2, y2);
	interfaces::surface->set_drawing_color(0, 0, 0, 255);
	interfaces::surface->draw_line(x1 + 1, y1, x2 + 1, y2);
	interfaces::surface->draw_line(x1 - 1, y1, x2 - 1, y2);
}
void render::draw_border_line_horizontal(std::int32_t x1, std::int32_t y1, std::int32_t x2, std::int32_t y2, color colour) {
	interfaces::surface->set_drawing_color(colour.r, colour.g, colour.b, colour.a);
	interfaces::surface->draw_line(x1, y1, x2, y2);
	interfaces::surface->set_drawing_color(0, 0, 0, 255);
	interfaces::surface->draw_line(x1, y1+1, x2, y2+1);
	interfaces::surface->draw_line(x1, y1-1, x2, y2-1);
}
